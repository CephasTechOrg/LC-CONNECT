...
- [x] No mobile work this phase — Phase 6's own checklist has no "Mobile:" bullet, and (per the Phase 4 decision) no employer-facing web portal exists yet to build this UI into; that portal remains a distinct, unscoped frontend project. *(Built afterward — see [Employer-facing web portal](#employer-facing-web-portal-employer-portal--2026-07-31) below.)*
...
### Employer-facing web portal (`employer-portal/`) ✅ (2026-07-31)
- [x] **Closes the Phase 4/5/6 flag**: a standalone Next.js 15 app (sibling to `admin/`, `backend/`, `mobile/`), mirroring `admin/`'s config (`@supabase/ssr`, App Router route groups, TypeScript, ESLint) so it fits the existing frontend conventions rather than inventing new ones.
- [x] New backend endpoint `GET /employers/me` (`MyEmployerRead`) — the portal's session-check call, mirroring the admin portal's `bootstrapUser` pattern but resolved against `EmployerAccount`/`EmployerOrganization`. Succeeding at all means the caller is an *approved* employer; pending/rejected 403 with the same messages Phase 5's auth layer already defined. Test added (`test_get_my_employer_context_shape`); snapshot regenerated additive-only (backend 399 passed).
- [x] Auth: Supabase email/password sign-in, deliberately **no MFA check** (unlike the admin portal's `aal2` gate) — employers aren't admins and Phase 3's MFA requirement was scoped to admin accounts only. Registration page posts unauthenticated to the existing `POST /employers/register`; no immediate login after registering, since `EmployerAccount.auth_user_id` stays NULL until Honors approval invites them (Phase 4 behavior, unchanged).
- [x] Dashboard: real KPIs only, computed from `GET /employers/scholars` (count) and `GET /employers/opportunities/me` (submissions grouped by status) — no fabricated metrics. The mockup's "Applications Received" concept was **deliberately dropped**: no applicant-tracking system exists or is in scope (§9 exclusion), so the KPI it would represent doesn't correspond to anything real; substituted a "Total Submissions" count instead. The Opportunity Status table shows the 3 real statuses (`pending`/`approved`/`rejected`) `EmployerOpportunitySubmission.status` actually supports, not the mockup's 4 (no `draft`/`expired` concept exists).
- [x] Scholar Directory + detail page — list from `GET /employers/scholars`, detail from `GET /employers/scholars/{user_id}` (server-side records an `EmployerProfileView` audit row per Phase 6 design — fetched once per page load, never polled or pre-fetched). Headshot/résumé are **not** pre-fetched: "View résumé" only calls `GET /employers/scholars/{user_id}/resume-url` on click and opens the short-lived signed URL immediately, matching the backend's signed-URL-per-request design (Phase 6) rather than caching a URL that may expire or a consent that may have been revoked since page load.
- [x] Opportunities page — list of the org's own submissions (status badges, reviewer note if rejected) plus a create form posting to `POST /employers/opportunities`, category restricted to the same 4 values the backend schema validates (`internship`/`job`/`volunteer`/`leadership`) so a client-side typo can't produce a 422 the user doesn't understand.
- [x] Organization page — org name/status/contact info via the same `GET /employers/me` call; pending/rejected states get a plain-language explanation instead of a bare status string.
- [x] Design: dark navy sidebar, light-gray page background, white rounded KPI/panel cards — matches `ui_mockups/employer mockup inspiration/` while reusing LC Connect's existing brand blue for accents (not a new brand).
- [x] Verification: `npm run build` (typecheck + Next.js lint) and `npm run lint` both clean; repo-wide `check_line_limits.py` unaffected (script covers `.py`/`.dart` only, per its existing scope — no new backend/mobile files crossed a threshold).
- [x] **Not built, still correctly out of scope**: no applicant-tracking UI (no backend concept to back it), no Notifications/Settings/Support nav items (no backend support for any of them yet) — omitted rather than stubbed with fake data.
- [x] **Final sweep caught and fixed two real gaps**: the scholar detail page had `has_headshot` in its type but never rendered a way to view it — only résumé was wired up, leaving the `GET /employers/scholars/{user_id}/headshot-url` endpoint completely unused by the frontend; fixed by generalizing to an `openDocument('headshot' | 'resume')` handler gated independently on each `has_*` flag. `discovery.resume_signed_url` (backend) had zero test coverage despite being structurally identical to the fully-tested `headshot_signed_url`; added the matching 3-test shape. Backend suite: 402 passed (was 399).
