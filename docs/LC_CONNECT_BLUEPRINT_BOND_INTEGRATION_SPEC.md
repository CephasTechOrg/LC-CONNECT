...
### Phase 2 — Scholar mobile experience and professional profile
- [ ] `ScholarProfessionalProfile` model: professional headshot ref, résumé file ref, LinkedIn, Handshake, summary, skills, career interests, `employer_visibility_consent` flag — kept fully separate from the existing `Profile`/social avatar.
- [ ] `StudentDisclosureConsent` model or field capturing consent timestamp/version (so a later policy change can require re-consent).
- [ ] **Résumé and headshot storage** (genuinely new infra — see §11.0, no existing precedent to copy): a private (non-public) Supabase Storage bucket, never a public bucket; every read goes through a short-lived **signed URL**, generated server-side, only for the profile owner, an approved employer with active consent, or an Honors admin. Validate file type and size **server-side** before accepting an upload — never trust the client, mirroring the `sanitize_avatar` validation pattern. On profile deletion or consent revocation, orphaned files get cleaned up (no dangling signed-accessible files after access should be gone).
- [ ] Rate-limit the upload endpoint with the existing `UserRateLimit` dependency (`app/shared/rate_limit.py`) — a résumé upload loop shouldn't be free.
- [ ] Backend endpoints (upload résumé/headshot, edit professional fields, toggle consent) gated behind "is verified scholar" — 403 for everyone else, enforced in `service.py`, not just hidden in the app.
- [ ] Error handling: `422` with a clear message for wrong file type/oversized file (not a generic 500); `503` if the storage client is unconfigured; `403` for a non-scholar hitting any of these endpoints directly.
- [ ] Mobile: Blueprint Bond completion card in Profile and Campus Hub, shown only when `ProgramMembership` is active.
- [ ] Mobile: professional-fields edit screen + résumé/headshot upload flow + consent toggle, following the existing `features/<x>/{providers,screens,widgets}` layout; upload failures show a real error state, not a silent no-op.
- [ ] Tests: non-scholar 403s on every professional endpoint; scholar can complete and save the extension; employer visibility never turns on without explicit consent; oversized/wrong-type upload rejected server-side even if the client would have blocked it too.
