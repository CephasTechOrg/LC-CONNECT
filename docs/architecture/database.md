...
- Real-time delivery uses Supabase Realtime (WebSocket). No polling needed. See `docs/architecture/realtime-messaging.md`.
- RLS is enabled on this table. Only match participants can read their messages. Direct client-side inserts are blocked. See `docs/security/rls_messages.md`.
