...
class EmployerAccount(Base):
    ...
    # Employer Agreement acceptance. `0` means never accepted — the server default, so every
    # existing employer account is gated on next sign-in rather than assumed to have agreed.
    agreement_accepted_version: Mapped[int] = mapped_column(
