# LC Connect — MVP To-Do Roadmap

This roadmap is designed to build the MVP in the correct order without making the app too complicated early.

## Phase 0 — Product Setup

- [ ] Confirm final app name: LC Connect, BlueBridge, CampusBridge, or another name
- [ ] Confirm official school spelling and branding usage
- [ ] Define primary MVP audience: Livingstone College students
- [ ] Confirm MVP feature list
- [ ] Decide whether first launch is private beta or open campus pilot
- [ ] Decide student verification method for MVP
- [ ] Create GitHub repository
- [ ] Add documentation folder
- [ ] Add this documentation to the repository

## Phase 1 — Design Planning

- [ ] Sketch main screens
- [ ] Define mobile navigation tabs
- [ ] Define color palette
- [ ] Define profile card design
- [ ] Define activity card design
- [ ] Define onboarding flow
- [ ] Define empty states
- [ ] Define safety/report screens

Required MVP screens:

- [ ] Welcome screen
- [ ] Register screen
- [ ] Login screen
- [ ] Onboarding/profile setup screen
- [ ] Home screen
- [ ] Connect/discovery screen
- [ ] Activity board screen
- [ ] Activity detail screen
- [ ] Create activity screen
- [ ] Messages screen
- [ ] Chat thread screen
- [ ] Profile screen
- [ ] Edit profile screen
- [ ] Report/block screen

## Phase 2 — Backend Setup

- [ ] Create FastAPI backend project
- [ ] Create virtual environment
- [ ] Install FastAPI, Uvicorn, SQLAlchemy, Alembic, Pydantic, PostgreSQL driver
- [ ] Create `.env.example`
- [ ] Configure database connection
- [ ] Add health check endpoint
- [ ] Set up API versioning: `/api/v1`
- [ ] Set up CORS for mobile development
- [ ] Set up password hashing
- [ ] Set up JWT authentication

## Phase 3 — Database Setup

- [ ] Create PostgreSQL database
- [ ] Configure Alembic migrations
- [ ] Create `users` table
- [ ] Create `profiles` table
- [ ] Create `interests` table
- [ ] Create `user_interests` table
- [ ] Create `languages` table
- [ ] Create `user_languages` table
- [ ] Create `looking_for_options` table
- [ ] Create `user_looking_for` table
- [ ] Create `connection_requests` table
- [ ] Create `matches` table
- [ ] Create `messages` table
- [ ] Create `activities` table
- [ ] Create `activity_participants` table
- [ ] Create `blocks` table
- [ ] Create `reports` table
- [ ] Create `verification_requests` table
- [ ] Add indexes
- [ ] Seed interests, languages, and looking-for options

## Phase 4 — Auth API

- [ ] Register endpoint
- [ ] Login endpoint
- [ ] Current user endpoint
- [ ] Password hashing
- [ ] JWT token generation
- [ ] JWT token validation
- [ ] Basic user status check
- [ ] Prevent suspended users from logging in or using core endpoints

Endpoints:

- [ ] `POST /api/v1/auth/register`
- [ ] `POST /api/v1/auth/login`
- [ ] `GET /api/v1/auth/me`

## Phase 5 — Profile API

- [ ] Create profile model
- [ ] Create profile schemas
- [ ] Create profile update endpoint
- [ ] Create get my profile endpoint
- [ ] Add interests to profile
- [ ] Add languages to profile
- [ ] Add looking-for preferences to profile
- [ ] Add profile completion logic
- [ ] Add hide profile setting

Endpoints:

- [ ] `GET /api/v1/profiles/me`
- [ ] `PATCH /api/v1/profiles/me`
- [ ] `GET /api/v1/profiles/{profile_id}`

## Phase 6 — Mobile App Setup

- [ ] Create Expo React Native app
- [ ] Add TypeScript
- [ ] Add Expo Router
- [ ] Add API client
- [ ] Add secure token storage
- [ ] Add basic UI components
- [ ] Add app theme/colors
- [ ] Add navigation tabs
- [ ] Connect app to backend health check

## Phase 7 — Mobile Auth Screens

- [ ] Welcome screen
- [ ] Register screen
- [ ] Login screen
- [ ] Store token securely
- [ ] Load current user on app start
- [ ] Redirect unauthenticated users to login
- [ ] Redirect new users to onboarding

## Phase 8 — Onboarding/Profile Setup

- [ ] Build profile setup form
- [ ] Add display name input
- [ ] Add major input
- [ ] Add class year picker
- [ ] Add country/state input
- [ ] Add short bio input
- [ ] Add interests selector
- [ ] Add languages spoken selector
- [ ] Add languages learning selector
- [ ] Add looking-for selector
- [ ] Submit profile to backend
- [ ] Show profile completion success

## Phase 9 — Discovery and Matching

- [ ] Create backend discovery service
- [ ] Exclude current user
- [ ] Exclude hidden/suspended users
- [ ] Exclude blocked users
- [ ] Exclude existing matches
- [ ] Calculate simple match reasons
- [ ] Return discovery cards
- [ ] Build mobile StudentCard component
- [ ] Add Connect action
- [ ] Add Study Together action
- [ ] Add Maybe Later action
- [ ] Add Report/Block action

Endpoint:

- [ ] `GET /api/v1/discovery/cards`

## Phase 10 — Connection Requests

- [ ] Create connection request endpoint
- [ ] Create incoming requests endpoint
- [ ] Create outgoing requests endpoint
- [ ] Create accept request endpoint
- [ ] Create decline request endpoint
- [ ] Create match after acceptance
- [ ] Prevent duplicate requests
- [ ] Prevent blocked user requests
- [ ] Build mobile requests screen
- [ ] Show match confirmation after accept

Endpoints:

- [ ] `POST /api/v1/connections/request`
- [ ] `GET /api/v1/connections/incoming`
- [ ] `GET /api/v1/connections/outgoing`
- [ ] `POST /api/v1/connections/{request_id}/accept`
- [ ] `POST /api/v1/connections/{request_id}/decline`
- [ ] `GET /api/v1/connections/matches`

## Phase 11 — Messaging

- [ ] Create message thread endpoint
- [ ] Create send message endpoint
- [ ] Enforce match-only messaging
- [ ] Enforce block restrictions
- [ ] Build messages list screen
- [ ] Build chat thread screen
- [ ] Add simple message bubbles
- [ ] Add timestamps
- [ ] Add empty state for no messages

Endpoints:

- [ ] `GET /api/v1/messages/threads`
- [ ] `GET /api/v1/messages/threads/{match_id}`
- [ ] `POST /api/v1/messages/threads/{match_id}`

## Phase 12 — Activities Board

- [ ] Create activity model
- [ ] Create activity schemas
- [ ] Create activity endpoint
- [ ] Create list activities endpoint
- [ ] Create activity detail endpoint
- [ ] Create join activity endpoint
- [ ] Create leave activity endpoint
- [ ] Add max participant logic
- [ ] Build activity board screen
- [ ] Build activity detail screen
- [ ] Build create activity screen
- [ ] Show joined activities

Endpoints:

- [ ] `POST /api/v1/activities`
- [ ] `GET /api/v1/activities`
- [ ] `GET /api/v1/activities/{activity_id}`
- [ ] `POST /api/v1/activities/{activity_id}/join`
- [ ] `DELETE /api/v1/activities/{activity_id}/leave`

## Phase 13 — Safety Features

- [ ] Create block endpoint
- [ ] Create unblock endpoint
- [ ] Create report endpoint
- [ ] Hide blocked users from discovery
- [ ] Prevent blocked messaging
- [ ] Add report button to profiles/cards
- [ ] Add block button to profiles/cards
- [ ] Add report reason picker
- [ ] Add safety confirmation messages

Endpoints:

- [ ] `POST /api/v1/blocks/{user_id}`
- [ ] `DELETE /api/v1/blocks/{user_id}`
- [ ] `POST /api/v1/reports`

## Phase 14 — Admin MVP

The admin panel can be simple at first. It can be web-based later.

- [ ] Create admin role
- [ ] Create admin-only permission check
- [ ] List users
- [ ] View reports
- [ ] Suspend user
- [ ] Remove activity
- [ ] Review verification requests

Endpoints:

- [ ] `GET /api/v1/admin/users`
- [ ] `GET /api/v1/admin/reports`
- [ ] `POST /api/v1/admin/users/{user_id}/suspend`
- [ ] `POST /api/v1/admin/activities/{activity_id}/remove`

## Phase 15 — Testing

Backend tests:

- [ ] User can register
- [ ] User can login
- [ ] User can create profile
- [ ] Discovery excludes blocked users
- [ ] Connection request can be sent
- [ ] Connection request can be accepted
- [ ] Match is created after accept
- [ ] User cannot message without match
- [ ] User cannot message after block
- [ ] User can create activity
- [ ] User can join activity
- [ ] User cannot join full activity
- [ ] Report can be submitted

Mobile testing:

- [ ] Register flow works
- [ ] Login flow works
- [ ] Profile setup works
- [ ] Discovery cards load
- [ ] Connection request works
- [ ] Messages screen works
- [ ] Activity board works
- [ ] Report/block works

## Phase 16 — Deployment

Backend:

- [ ] Deploy FastAPI backend to Render, Railway, Fly.io, or similar
- [ ] Deploy PostgreSQL database
- [ ] Configure environment variables
- [ ] Run migrations in production
- [ ] Set allowed CORS origins
- [ ] Add production logging

Mobile:

- [ ] Configure Expo app settings
- [ ] Create development build
- [ ] Test on Android phone
- [ ] Test on iPhone if available
- [ ] Prepare app icon and splash screen
- [ ] Prepare internal testing build

## Phase 17 — Pilot Launch

- [ ] Invite small group of students first
- [ ] Collect feedback
- [ ] Track signup rate
- [ ] Track profile completion
- [ ] Track connection requests
- [ ] Track match rate
- [ ] Track activity joins
- [ ] Watch for safety issues
- [ ] Fix bugs quickly
- [ ] Improve onboarding based on feedback

## Phase 18 — Future Features After MVP

Only add these after students are actually using the MVP:

- [ ] Push notifications
- [ ] Real-time messaging with WebSockets
- [ ] AI-generated icebreakers
- [ ] Better recommendation engine
- [ ] Club/organization pages
- [ ] Group chats for activities
- [ ] Event approval flow
- [ ] Campus map
- [ ] Admin dashboard UI
- [ ] Web landing page
- [ ] Analytics dashboard
- [ ] Photo moderation
- [ ] Better search filters

## Build Priority Summary

Build first:

1. Auth
2. Profile setup
3. Discovery cards
4. Connection requests
5. Matches/messages
6. Activities
7. Report/block

Everything else can wait.
